...
module "private_ec2" {
  ...
  user_data = <<EOF
    ...
    RDS_MYSQL_ENDPOINT="${element(split(":", aws_db_instance.ketav_RDS.endpoint), 0)}";
    RDS_MYSQL_USER="ketavbhatt";
    RDS_MYSQL_PASS="Qwerty12";
    RDS_MYSQL_BASE="ketav-db";
    mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE -e 'quit';
    if [[ $? -eq 0 ]]; then
      echo "MySQL connection: OK" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='CREATE TABLE ketav1( id INT)';
      echo "MySQL table create" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='INSERT into ketav1 (id) VALUES (1)';
      echo "MySQL insert" >> conn;
      mysql -h $RDS_MYSQL_ENDPOINT -u $RDS_MYSQL_USER -p$RDS_MYSQL_PASS -D $RDS_MYSQL_BASE --execute='SELECT * from ketav1' >> conn;
...
module "ketav_RDS" {
  ...
  subnet_group_name = "ketav_subnet_group"
